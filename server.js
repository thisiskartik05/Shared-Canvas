...
app.get("/whiteboard/:room", (req, res) => {
  ...
  if (boardroomSet.has(roomId)) {
    if (boardroomSet.get(roomId)[1] == req.session.user.email)
      return res.render("whiteboard-admin.ejs", {
        roomId,
...
io.on("connect", (socket) => {
  ...
  socket.on("join-board", (roomId, name) => {
    ...
    boardroomSet.set(roomId, [
      ...
      boardroomSet.get(roomId)[1],
      null,
    ]);
    socket.broadcast.to(roomId).emit("notify", name);
    socket.on("draw", (data) => {
      boardroomSet.set(roomId, [
        boardroomSet.get(roomId)[0],
        boardroomSet.get(roomId)[1],
      ]);
